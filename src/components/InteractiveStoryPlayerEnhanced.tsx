...
interface InteractiveStoryPlayerEnhancedProps {
  storyId: string
  onBack?: () => void
}

const InteractiveStoryPlayerEnhanced = ({ storyId, onBack }: InteractiveStoryPlayerEnhancedProps) => {
  ...
  const [currentTime, setCurrentTime] = useState(0) // Track story playback time in seconds
  ...
  const startTimeTracking = () => {
    ...
    timeInterval.current = setInterval(() => {
      if (isPlaying && !playbackController.current.shouldStop) {
        const elapsed = (Date.now() - storyStartTime.current) / 1000
  ...
  const playStoryWithInteractions = async () => {
    ...
    console.log('🎬 Starting enhanced story playback from sentence:', currentSentenceIndex)
    setIsPlaying(true)
    ...
    try {
      for (let i = currentSentenceIndex; i < sentences.current.length; i++) {
        // Check if we should stop playbook
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-kidGreen via-kidBlue to-kidPurple">
      ...
      <VoiceInteractionHandler
        ...
        childProfileId="demo-profile"
      />
    </div>
  )
}

export default InteractiveStoryPlayerEnhanced
